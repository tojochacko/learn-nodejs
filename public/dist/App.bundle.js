...
function ajaxHeart(e) {
  ...
  _axios2.default.post(url).then(function (response) {
    ...
    (0, _bling.$)('.heart-count').textCount = response.data.hearts.length;
    if (isHearted) {
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // zoom the map to fill all markers perfectly
    map.setCenter(bounds.getCenter());
    map.fitBounds(bounds);

    // when someone clicks on the marker show the details of that page
    markers.forEach(function (marker) {
      marker.addListener('click', function () {
        var html = '\n          <div class="popup">\n            <a href="/store/' + this.place.slug + '">\n              <img src="/uploads/' + (this.place.picture || 'store.png') + '" \n              alt="' + this.place.name + '" />\n              <p>' + this.place.name + ' - ' + this.place.location.address + '</p>\n            </a>\n          </div>\n        ';
...
function searchResultsHTML(searchData) {
  return searchData.map(function (store) {
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        ...
        searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results \n                                    for <strong>' + _this.value + '</strong> found!</div>');
      }
    }).catch(function (err) {
      console.log(err);
    });
  ...
  // check for specific keyboard buttons
  searchInput.on('keyup', function (e) {
    ...
    // if they are pressing up, down or enter, who cares!
